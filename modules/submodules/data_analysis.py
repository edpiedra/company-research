...
class DataAnalysis():
    ...
    def _run_through_nlp_models(self):
        ...
        self.fingpt.file_contents = self.file_contents 
        self.fingpt.get_sentiment()
            
    def _company_news(self):
        print("[INFO] {:.2f}...analyzing company news".format(
            time.time()-start_time
        ))
        ...
        for article in news:
            ...
            results["fingpt_sentiment"].append(self.fingpt.sentiment)
        ...
        df.to_csv(self.target + "/company news.csv")
    ...
    def _earnings_calls(self):
        ...
        for filename in os.listdir(self.folder):
            ...
            if len(transcript) > 0:
                ...
                final["fingpt"].append(df["fingpt_sentiment"].mode())
        ...
        df["period"] = (df["quarter"] * 3).astype(str) + df["year"]
